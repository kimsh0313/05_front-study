...
var userId;
console.log(userId);      // 특징1. 할당건 : undifined (초기화 없이 선언 가능)
...
// let userName = '홍길동'; // 틍징2. 재선언 : 불가능 (동일 스코프에 중복 선언 허용X)
userName = '홍길동';
userName = '고길동';
...
// const hooby            // 특징1. 할당건 : 사용 불가능 (초기화를 해야만 사용 가능)
const hobby = '요리';
// const hobby = '등산';  // 특징2. 재선언 : 불가능

// hobby = '등산';        // 특징3. 재할당 : 블가능
...
function constScopeTest(){
  ...
  console.log(dream);
  //console.log(userAge);     // 특징4. scope : 블럭스코프
}
...
console.log(x);
console.log(y);
console.log(nokeyword); // 함수내에 ver|let|const 키워드 없이 변수 선언시 => 전역변수로 설정됨
