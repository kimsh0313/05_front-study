...
*/
//익명함수를 사용하는게 일반적임, 기명함수도 쓸수는 잇음, 그러나 다시 재호출 불가
(function(){
  console.log('즉시 실행함수! 함수 정의와 동시에 호출');
})();
...
function outer(a){
  console.log("외부함수", a);
  inner(a+1); //내부 함수도 호이스팅 -> 외부 함수 내의 최상단에 끌어올려져있음
 ...
 // console.log(b); 외부함수에서 내부함수 변수 사용불가능
}
outer(10);
...
/*
  ...
  b 함수 == 고차함수 
*/
//case 1. 경우에 따라 특정값을 증가 또는 감소를 시켜줘야 되는 케이스
...
// 값에 변화를 주는 함수
function apply(func, value){   //첫번쨰 인자로는 어떻게 변화시킬껀지의 함수, 두번재 인자로는 변화시킬 값
  return func(value);
}
...
//case2. 로그인을 처리할 때 아이디/비번/이메일/토큰 중에 매번 두개를 체크해야된다고 가정
