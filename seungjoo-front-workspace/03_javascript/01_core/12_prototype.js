...
const student = {
  passion:true,
  //__proto__ : user //이걸 통해 부모 객체를 할당할 수 있음
};
...
Object.setPrototypeOf(ssgStundent,student);
console.log(ssgStudent.academy);
console.log(ssgStudent.passion); //상속받고 있는 student의 property
...
function EV(model){  //생성자 함수
  this.model = model;
}

EV.prototype = car;

const ev9 = new EV('EV9');  //생성자 함수를 통한 인스턴스 생성
console.log(ev9);
...
function Animal(name){
  this.name = name;    //생성자 함수 내부에 메소드 정의
  this.speak = function(){
    console.log('${this.name}이 짖습니다.');
  }
}
//Animal.prototype = ={contructor:Animal}
console.log(Animal.prototype.Constructor == Animal);

//보통 해당 생성자 함수로 생성되는 객체들이 사용할 메소드(인스턴스 메소드)를 프로토타입으로 정의
Animal.prototype.speak = function(){
  console.log('${this.name}이 짖습니다.'); //자기 자신의 prototype 객체에 메소드 정의
};

const dog = new Animal('멍멍이');
const cat = new Animal('야옹이');

//Animal.proptotype.speak : Animal 객체로 호출 가능한 인스턴스 메서드
dog.speak();
cat.speak();
