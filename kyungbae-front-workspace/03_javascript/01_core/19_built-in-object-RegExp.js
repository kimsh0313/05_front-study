...
/*
  ...
  3. 주요 메타 문자
    ...
    2) * : 0개 이상의 문자를 의미
          ex) good*   → (O) goo, good, goodd, gooddd // d가 0개 또는 이상

    3) + : 1개 이상의 문자를 의미
          ex) good+   → (O) good, gooddd / (X) goo // d가 1개 이상
...
// g로 시작하고 d로 끝나는데 그 사이에 o문자가 2글자 이상인지 비교
regExp = /^go{2,}d$/;
...
// 시작부터 끝까지 숫자로만 이루어진 문자열인지 비교
regExp = /^[0-9]+$/;
console.log(regExp.test(12041305015));
console.log(regExp.test(`12041가305015`));

// 시작부터 끝까지 영문자로만 이루어진 문자열인지 비교
regExp = /^[a-zA-Z]+$/;
console.log(regExp.test('JavaScript'));
console.log(regExp.test('Java Script'));

// 시작부터 끝까지 한글로만 이루어진 문자열인지 비교
regExp = /^[ㄱ-ㅎㅏ-ㅣ가-힣]+$/;
console.log(regExp.test('간하ㅣㄹ아즈힞이'));
...
const nameCheck = ['홍길동', '김필','James', '김따오기하늘매잡아', '김john', '김다섯글자'];
nameCheck.forEach(a => verifyName(a));
...
const zipcodes = ['12345', '1515', 'abcdd', 10101, '654321', '08120'];
zipcodes.forEach(zipcode => verifyZipcode(zipcode));
