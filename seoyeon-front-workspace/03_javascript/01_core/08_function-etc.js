...
// 익명 함수를 사용하는게 일반적임
(function() {
  console.log('즉시 실행 함수! 함수 정의와 동시에 호출');
})();

 // 기명함수도 즉시 실행 가능하지만 다시 재호출이 불가능
(function hello(name) {
...
function outer(a){
  ...
  // console.log(b);               // 외부함수에서 내부함수 변수 사용 붉가
}
...
// case 1. 경우에 따라 특정 값을 증가 도는 감소를 시켜줘야되는 케이스
...
function apply(func, value){ // 첫 번째 인자로는 어떻게 변화시킬껀지의 함수, 두 번째 인자로는 변화시킬 값
  return func(value);
}
...
// 로그인 처리용 함수 정의 (매번 경우에 따라 체크해야될 두 개의 함수를 콜백함수로 전달받기)
const login = (check1, check2) => {
  console.log('로그인 처리를 위한 공통 로직 먼저 실행');
  // 검증절차를 위해 전달된 두 개의 체크용 함수 실행
  check1();
  check2();
};
